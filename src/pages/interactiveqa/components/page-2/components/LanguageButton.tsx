...
export default function LanguageButton({ functions, selected, language, currLanuage, viewLanguage }: ILanguageButton) {
  return (
    <button
      onClick={functions}
      className={`w-full h-20 md:w-[550px] md:h-[60px] 3xl:w-[800px] 3xl:h-[150px]  flex flex-row items-center justify-start gap-5 md:gap-[32px] px-3 md:px-[33px] border-2 border-[#35D36A] rounded-lg ${language == currLanuage ? "bg-[#35D36A] " : ""} ${selected == ((currLanuage == "english") ? 0 : 1) ? "border-2 border-white" : ""}`}>
      <img
