...
export default function MintBadge({ setPage }: Props) {
  ...
  return (
    <div
      ...
      <div className="w-full h-full flex flex-col items-center justify-start gap-5 md:gap-[52px]">
        <p className="text-[#63E4B7] text-4xl md:text-[64px] font-bold">Power Saver Pro</p>
        <p className="text-xl md:text-[28px]">Congrats, You Got the “Waste - Warrior” Badge!</p>
      </div>
...
const ProgressBar: React.FC<ProgressBarProps> = ({ percentage }) => {
  ...
  return (
    <div className="flex overflow-hidden relative flex-col md:mt-3.5 w-full rounded-2xl min-h-[10px] md:min-h-[18px] max-md:max-w-full">
      ...
      <div
        className="flex absolute bottom-0 left-0 z-0 max-w-full bg-[#63E4B7] rounded-full h-[10px] md:h-[18px] md:min-h-[18px]"
        style={{ width }}
      />
    </div>
  );
