...
export default function PageSixteen({ setPage }: Props) {
  const [selectedItems, setSelectedItems] = useState<string[]>(["none"]); // Set "none" as selected by default
  ...
  useEffect(() => {
    const recycleHabit = selectedItems.includes("none") || selectedItems.length === 0 ? "no" : "yes";
    const recycleMaterials = recycleHabit === "no" ? [] : selectedItems;

    // Emit data to the server
    socket?.emit(
      "page-change-send-data-server",
      JSON.stringify({
        room: room,
        page: "page-16",
    ...
    dispatch(
      addWaste({
        ...
        recycleMaterials: recycleMaterials, // Ensure this matches the schema
      })
    );
  }, [selectedItems]);

  return (
    <DefaultBackground currPage={16}>
      <div className="w-full h-full relative z-10">
        {/* Title */}
        <div className="w-full h-auto flex flex-col items-start justify-start pl-40 text-white pt-[250px]">
          <div className="flex flex-row items-center justify-start gap-5">
            <div className="w-10 h-3 bg-purple-500"></div>
            <p className="text-white text-[60px]">
              <CarbonLanguage name="waste_disposal" />
            </p>
          </div>
          <p className="text-[40px]">
            <CarbonLanguage name="recycling_habits" />
          </p>
          <p className="text-[20px] text-gray-400 mt-2">
            <CarbonLanguage name="which_of_the_following_materials_do_you_recycle" />
          </p>
        </div>

        {/* Options */}
        <div className="w-full h-auto flex flex-col items-start justify-start pl-40 pt-20 gap-10">
          {checkboxItems.map((item) => (
            <CheckboxComponent
              ...
              onToggle={(type) => {
                if (type === "none") {
                  setSelectedItems(["none"]); // Reset to only "none" when "none" is selected
                } else {
                  setSelectedItems((prev) => {
                    const withoutNone = prev.filter((t) => t !== "none"); // Remove "none" if another item is selected
                    return prev.includes(type)
                      ? withoutNone.filter((t) => t !== type) // Remove the selected type if already selected
                      : [...withoutNone, type]; // Add the selected type
                  });
                }
              }}
            />
        ...
        <div className="absolute bottom-0 right-0">
          <NavComponent setPage={setPage} nextPage={17} prevPage={15} />
        </div>
      </div>
    </DefaultBackground>
  );
  ...
  return (
    <div className="w-full h-full flex flex-col items-start justify-start gap-5 text-white">
      ...
      >
        ...
        />
        <p className="text-xl md:text-[45px] font-normal">{item.name}</p>
      </div>
    </div>
  );
};