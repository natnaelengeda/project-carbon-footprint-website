...
export default function Bicycle({ opened, setOpened }: Props) {
  ...
  useEffect(() => {
    const bicycles = transportationMode.filter((item: any) => item.name == "bicycle");

    if (bicycles.length != 0) {
      setSelected(true);

      if (bicycles) {
        setSlider1(bicycles[0].value ?? 1);
        setSliderMax1(bicycles[0].value ?? 1);
        setSlider2(bicycles[0].frequency ?? 1);
        setSliderMax2(bicycles[0].frequency ?? 1);
      }
